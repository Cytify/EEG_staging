...
def gp_analyse(data):
    stage_gp = {"W": [], "1": [], "2": [], "3": [], "4": [], "R": []}
    for file in data:
        curr_stage = {"W": [], "1": [], "2": [], "3": [], "4": [], "R": []}
        for sig in data[file]:
            eeg = sig[0]
            stage = sig[1]
            gp = grassberg_procaccia(np.asarray(eeg), 10, 23)
            print(gp)
            stage_gp[stage].append(gp)
            curr_stage[stage].append(gp)
    ...
    for stage in stage_gp:
        mean = np.mean(stage_gp[stage])
        std = np.std(stage_gp[stage])
        print(stage, "mean", mean, ", std", std)
